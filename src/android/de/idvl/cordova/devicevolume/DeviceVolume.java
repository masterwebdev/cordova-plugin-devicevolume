package de.idvl.cordova.devicevolume;
...
public class DeviceVolume extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        if ("getDeviceVolume".equals(action)) {
            triggerEvent(callbackContext, currentVolume(), false);
            return true;
        } else if ("setDeviceVolume".equals(action)) {
            int volume = args.getInt(0);
            changeVolume(volume);
            return true;
        } else if ("setDeviceVolumeChangeCallback".equals(action)) {
    ...
    private void changeVolume(int volume) {
        ...
        if (volume == 0) {
            audioManager.adjustStreamVolume(STREAM, AudioManager.ADJUST_LOWER, AudioManager.FLAG_REMOVE_SOUND_AND_VIBRATE);
        } else {
            audioManager.adjustStreamVolume(STREAM, AudioManager.ADJUST_RAISE, AudioManager.FLAG_REMOVE_SOUND_AND_VIBRATE);
        }
    }
